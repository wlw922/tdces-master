...
class alltogetherImageCell: UITableViewCell {

 @IBOutlet weak var labQuestionTitle: UILabel!
 @IBOutlet weak var labQuestionImage: UIImageView!
 @IBOutlet weak var answerTV: UITextView!
    ...
    func setupWithBean(examContentBean : ExamContentBean) {
      ...
      //问题图片
        let questionImageUrl:String = PHOTO_URL + examContentBean.fileUrl!

        labQuestionImage.kf.setImage(with: URL.init(string: questionImageUrl), placeholder: nil, options: nil, progressBlock: nil, completionHandler: { (result) in
        })
        //答案
