...
extension TranscriptViewController {
    override func viewDidLoad() {
        ...
        if(transcriptList != nil){
            ...
            //获取试卷查询数据
            TrainBiz.sharedInstance.findExamListByStaff(idNumber: (staff?.idNumber)!) { (responseBean) in
...
extension TranscriptViewController:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        weak var weakSelf=self
        DispatchQueue.main.async {
            let transcriptDetailView = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "TranscriptDetailView") as! TranscriptDetailViewController
            transcriptDetailView.examId = String((weakSelf?.transcriptList[indexPath.row].id)!)
//            weakSelf?.present(transcriptDetailView, animated: true, completion: nil)
            weakSelf?.navigationController?.pushViewController(transcriptDetailView, animated: true)
        }
    }
